...
\RR	  \arrow{rr}{f}\arrow{dd}[anchor=east]{\id} && \RR\arrow{dd}[anchor=west]{\varphi} \\ 
	...
	\begin{align*}
	  ...
	  \begin{prop}[Ejercicio de Milnor]
...
\begin{proof}\leavevmode
  ...
  Sean $x,y \in M$, $x\neq y$. Para probar la inyectividad basta tomar una función separante $\theta\in C^{\infty}(M)$ tal que $\theta(x)=1$ y $\theta(y)=0$. Para probar la sobreyectividad haremos uso del siguiente resultado, cuya demostración puede encontrarse en el ejercicio I.3.9 de [Ruiz].

  \begin{lema}
    Sea $M$ una variedad diferenciable. Existe una función $f\in C^{\infty}(M)$ tal que todos sus conjuntos de nivel son compactos.
  \end{lema}
   Sea ahora $\xi: C^{\infty}(M)\rightarrow \RR$. Tomemos una función $f\in C^{\infty}(M)$ con superficies de nivel compactas y llamemos $\lambda=\xi(f)$. Supongamos que para ningún $x\in f^{-1}(\lambda)$ se tiene que $\bar{x}=\xi$. Entonces existe una familia de funciones $\left\{ f_x: x\in f^{-1}(\lambda) \right\}$ tal que $f_x(x)\neq \xi (f_x)$. Consideramos entonces un recubrimiento de $f^{-1}(\lambda)$ por la familia de abiertos $\left\{ U_x : x\in f^{-1}(\lambda) \right\}$, con $$U_x= \left\{ y\in M:f_x(y)\neq \xi(f_x) \right\}.$$
...
M=	    \left\{ x\in B: F(x)=F(a) \right\}
	  ...
	  Hemos definido el espacio tangente para variedades sumergidas, pero cabe preguntarse ahora si se puede hacer una construcción análoga para el caso de una variedad diferenciable cualquiera, en principio no sumergida. Esta construcción puede hacerse, usando el concepto de \textit{derivación} de un álgebra. La idea es considerar la variedad diferenciable como una $C^\infty$-variedad y tratar de estudiar el aspecto \textit{infinitesimal} del haz de estructura en un punto de la variedad. La manera de hacer este estudio es por medio de las \textit{espigas} del haz. 
	    ...
	    \begin{equation*}
	      \FF_x = \varinjlim_{p\in U \subset M} \FF(U).
	    \end{equation*}
	  \begin{ejemplo}
	    Un ejemplo de por qué la espiga de un haz codifica la información infinitesimal de éste aparece si estudiamos el caso \textit{analítico}. En este caso tomamos como espacio topológico simplemente la recta real $\RR$ y consideramos el haz $\mathscr{A}_\RR$ que a cada abierto $U\subset \RR$ le asigna el conjunto de las funciones analíticas $f:U\rightarrow \RR$. Es decir, que existe un punto $x_0\in U$ tal que
	    \begin{equation*}
	      f(x)=\sum_{n=0}^\infty a_n(x-x_0)^n,
	    \end{equation*}
	    para todo $x\in U$.
	    La espiga de $\mathscr{A}_\RR$ es simplemente el conjunto 
	    \begin{equation*}
	      \mathscr{A}_\RR= \left(\bigsqcup_{U \text{ entorno de } 0} \mathscr{A}_{\RR} (U)\right) /\sim,
	    \end{equation*}
	    con $\sim$ la relación de equivalencia que identifica dos funciones si y sólo si éstas coinciden en algún entorno de $0$.
	  \end{ejemplo}

	    En el caso en que nos incumbe, si $M$ es una variedad diferenciable de dimensión $n$ y $C^\infty_M$ denota su haz de funciones diferenciables, entonces la espiga de $C^\infty_M$ en un punto $x\in M$ es simplemente el conjunto de las funciones $f:M\rightarrow \RR$ que son diferenciables en algún entorno de $x$. Este conjunto se denota $C^\infty_{M,x}$ y se llama el \emph{álgebra de gérmenes de funciones diferenciables en $x$}. Cada conjunto $C^\infty_M(U)$ es una $\RR$-álgebra, donde las operaciones son simplemente la suma y el producto de funciones. Por tanto, las espigas $C^\infty_{M,x}$ también serán $\RR$-álgebras. 
